...
export default function CsvImporter() {
  ...
  return (
    <>
      <h3 style={{ marginBottom: 4 }}>Importar clientes desde CSV</h3>
      <p style={{ color: 'var(--muted)', fontSize: 13, marginBottom: 16 }}>
        Arrastra tu archivo CSV o pega los datos directamente. Se agregan a tu CRM sin duplicados y respetando la LOPDP.
      </p>
      <form action={importContacts} onSubmit={() => setLoading(true)}>
        {/* Zona drag & drop */}
        <div
        ...
        >
          ...
          {filename ? (
            <div>
              ...
              {rowCount !== null && (
                <div style={{ fontSize: 13, color: '#15803d', fontWeight: 600, marginBottom: 8 }}>
                  ✅ {rowCount} {rowCount === 1 ? 'contacto detectado' : 'contactos detectados'}
                </div>
              )}
              <button type="button" onClick={handleReset} style={{ background: '#475569', padding: '6px 14px', fontSize: 12 }}>
                Cambiar archivo
              </button>
            </div>
          ) : dragging ? (
            ...
            <div>
              ...
              <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 4 }}>Arrastra tu archivo CSV aquí</div>
              <div style={{ fontSize: 13, color: 'var(--muted)' }}>o haz clic para seleccionar · CSV, TXT</div>
            </div>
          )}
        </div>
        ...
        <div style={{ marginBottom: 16 }}>
          <label style={{ marginBottom: 6 }}>
            {filename ? 'Contenido del archivo (puedes editarlo antes de importar)' : 'O pega aquí los datos directamente'}
          </label>
          <textarea
            name="pasted"
            rows={csvText ? 6 : 4}
        ...
        <label style={{ display: 'flex', gap: 10, alignItems: 'flex-start', fontWeight: 500, fontSize: 13, cursor: 'pointer', marginBottom: 16 }}>
          <input type="checkbox" name="consent" value="1" style={{ width: 'auto', marginTop: 2, flexShrink: 0 }} />
          <span>
