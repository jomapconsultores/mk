...
export async function importContacts(formData: FormData) {
  ...
  text = text.trim();
  if (!text) redirect('/import?err=empty');

  const firstLine = text.split('\n')[0];
  const delim = firstLine.split(';').length > firstLine.split(',').length ? ';' : ',';
  const rows = parseCSV(text, delim);
  if (rows.length < 2) redirect('/import?err=empty');
  ...
  if (!items.length) redirect('/import?ok=0&dup=0');
  ...
  redirect(`/import?ok=${inserted}&dup=${dup}`);
}
